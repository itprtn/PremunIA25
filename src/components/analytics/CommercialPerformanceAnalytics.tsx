...
export function CommercialPerformanceAnalytics({ contacts = [], projets = [], contrats = [] }: CommercialPerformanceAnalyticsProps) {
  ...
  const performanceAnalytics = useMemo(() => {
    ...
    // Process contrats - try to link to projets when possible
    filteredContrats.forEach(contrat => {
        let assignedCommercial = 'Non attribué';

        // First, priority: use contrat's own commercial field if it has one
        if (contrat.commercial) {
            assignedCommercial = contrat.commercial;
        } else {
            // Second: try to find related projet and use its commercial
            const projet = filteredProjets.find(p => p.projet_id && contrat.projet_id &&
                String(p.projet_id) === String(contrat.projet_id));
            if (projet?.commercial) {
                assignedCommercial = projet.commercial;
            }
        }

        // Ensure commercial data entry exists
        if (!commercialData[assignedCommercial]) {
            commercialData[assignedCommercial] = {
                projets: 0,
                contrats: 0,
                chiffreAffaires: 0,
                commissions: 0,
        ...
        // Add contrat data to commercial
        commercialData[assignedCommercial].contrats++;
        commercialData[assignedCommercial].chiffreAffaires += contrat.prime_brute_annuelle || 0;
        commercialData[assignedCommercial].commissions += contrat.commissionnement_annee1 || 0;

        // If contrat is linked to a projet, mark that projet as "converted"
        if (contrat.projet_id) {
            const linkedProjet = filteredProjets.find(p =>
                p.projet_id && String(p.projet_id) === String(contrat.projet_id)
            );
            if (linkedProjet) {
                // Note: We're not changing projet counting logic here,
                // but marking the conversion relationship
            }
        }
    });
